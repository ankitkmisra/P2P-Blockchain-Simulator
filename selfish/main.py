...
class Simulation:
    def __init__(self, n, a, ttx, z0, z1, zeta, I=600):
        ...
        self.a = a
        self.zeta = zeta
        self.adversary = n-1  #always taking last to be adversary node
        initLatency(n)
    ...
    def print_stats(self):
        ...
        #find all blocks generated by adversary
        ablocks = []
        for _,block in nd.blockChain.items(): 
            if block.pb != 0 and (block.miner.nid == self.adversary): #if parent and miner exists
                ablocks+=[block.bid]   
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--num_nodes', default=50, type=int, help='number of nodes in the P2P network')
    parser.add_argument('-a', '--alpha', default=0.2, type=int, help='adversary mining power')
    parser.add_argument('-z0', '--percentage_slow', default=0.9, type=float, help='percentage of slow nodes')
    parser.add_argument('-z1', '--percentage_lowcpu', default=0.9, type=float, help='percentage of nodes having low CPU power')
